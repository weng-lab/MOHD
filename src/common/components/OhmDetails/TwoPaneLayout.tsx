...
const DownloadButton = ({ isXs, setModalOpen }: DownloadButtonProps) => {
  const onClick = () => {
    setModalOpen(true);
  };
  return isXs ? (
    <IconButton color="primary" aria-label="download" size="small" onClick={onClick}>
      <DownloadIcon />
    </IconButton>
  ) : (
    <Button variant="outlined" startIcon={<DownloadIcon />} onClick={onClick}>
      Download
    </Button>
  );
};
...
const TableIconButton = ({ tableOpen, handleToggleTable }: TableIconButtonProps) => (
    ...
    <Stack spacing={2} direction={{ xs: "column", lg: "row" }} id="two-pane-layout">
      ...
      >
        <Stack direction={"row"} alignItems={"center"} gap={1} mb={2}>
          <TableIconButton tableOpen={tableOpen} handleToggleTable={handleToggleTable} />
      ...
      <Box flex="1 1 0" minWidth={0} id="tabs_figure_container">
        <Stack direction={"row"} alignItems={"center"} justifyContent={"space-between"}>
          <Stack direction={"row"} alignItems={"center"} mb={2} gap={2}>
            {!tableOpen && <TableIconButton tableOpen={tableOpen} handleToggleTable={handleToggleTable} />}
          ...
          <DownloadButton isXs={isXs} setModalOpen={setModalOpen} />
        </Stack>
