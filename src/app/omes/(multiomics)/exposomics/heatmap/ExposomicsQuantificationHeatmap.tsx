...
const ExposomicsQuantificationHeatmap = ({
  ...
  const heatmapData: ColumnDatum<ExposomicsSample>[] = samples.map((sample) => {
    ...
    <OmeHeatmapShell
      ...
      tooltipBody={(bin) => (
        <>
          ...
          {bin.bin.metadata?.formula ? (
            <Typography>
              <b>Formula:</b> {bin.bin.metadata.formula}
            </Typography>
          ) : null}
          {bin.bin.metadata?.ionType ? (
            <Typography>
              <b>Ion Type:</b> {bin.bin.metadata.ionType}
            </Typography>
          ) : null}
          {bin.bin.metadata?.precursorMz != null ? (
            <Typography>
              <b>Precursor m/z:</b> {bin.bin.metadata.precursorMz}
            </Typography>
          ) : null}
          <Typography>
